...
# Use db push for initial install (pushes schema directly, no migration files needed).
# Migration files are generated by developers with: npx prisma migrate dev --name <name>
# Once migration files exist in prisma/migrations/, this will automatically switch to migrate deploy.
